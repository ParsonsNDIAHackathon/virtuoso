...
PROMPT_VERSION = "fusion-evidence-v1"
...
ASSESSMENT_SCHEMA = {
    ...
    "required": ["verdict", "relation", "evidence_strength", "supporting_facts", "strongest_limitation", "rationale", "resolved_entities"],
}
...
class FusionAI:
    ...
    def adjudicate(self, candidate: Candidate) -> Assessment:
        cache_key = f"assessment:{PROMPT_VERSION}:{self.client.model}:{candidate.id}"
        cached_assessment = self.cached_assessment(candidate)
        if cached_assessment:
            return cached_assessment
        payload = {
            "candidate_features": candidate.to_dict(include_records=False),
            "record_a": candidate.left.to_dict(), "record_b": candidate.right.to_dict(),
        }
        instructions = (
            ...
            "identity/action link is missing and an analyst must review it. Use INSUFFICIENT_EVIDENCE when proximity "
        ...
        assessment = Assessment(
            ...
            evidence_strength=round(max(0.0, min(1.0, float(result["evidence_strength"]))), 3),
    ...
    def clusters(self, assessments: Iterable[Assessment]) -> list[FusionCluster]:
        ...
        for start in adjacency:
            ...
            strength = sum(a.evidence_strength for a in related) / max(1, len(related))
            score = round(min(1.0, 0.18 * len(modalities) + 0.55 * strength + 0.05 * min(len(related), 4)), 3)
    ...
    def brief(self, cluster: FusionCluster, assessments: Iterable[Assessment]) -> FusionCluster:
        ...
        cached = self.cache.get(cache_key)
        if cached:
            cluster.brief, cluster.caveats = cached["brief"], cached["caveats"]
            return cluster
        ...
        cluster.caveats = [str(value)[:400] for value in result.get("caveats", [])[:5]]

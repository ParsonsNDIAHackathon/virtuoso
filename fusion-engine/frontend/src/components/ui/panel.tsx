...
function StructuredPanel({ className, children, panelId, openOn, ...props }: PanelProps) {
  ...
  useEffect(() => {
    ...
    // openOn: the panel just received new content (a selection changed), so a collapsed panel opens.
    if (openOn !== undefined) { setCollapsed(false); store.set(`${key}.collapsed`, null); return; }

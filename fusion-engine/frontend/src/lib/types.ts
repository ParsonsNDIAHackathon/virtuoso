...
export type IncidentExplanation = { id: string; title: string; supported: number; contradicted: number; untested: number; predictions: IncidentPrediction[] };
export type Incident = { id: string; cells: [number, number][]; first_t: number; last_t: number; state: "new_change" | "persistent" | "recovering"; streams: Record<string, { departed: boolean; adequate: boolean; best: Departure | null; detail?: Departure | null; reason?: string }>; explanations: IncidentExplanation[]; next_check: { prediction: string; separates: number; source: string; why: string } | null; revisions: Array<{ t: number; added: string[]; gone: string[]; cells: number; leading: string | null }>; assessment: { established: string; disputed: string; unresolved: string; leading: string | null; relevance: string; adequately_supported?: boolean; place?: string | null; headline?: string; changes?: string[]; operational_impact?: string; question?: string; next_action?: { question: string; source: string; area: string; window: [number, number]; mode: "automatic" | "recheck" | "analyst"; how: string; why: string } }; evidence?: IncidentEvidence[]; evidence_total?: number; evidence_scope?: string };
export type IncidentEvidence = { kind: string; stream: string; id: string; ts: string; title: string; codes?: string[]; text: string; url: string; place: string | null; lat: number; lon: number; mentions?: number; neighbour?: boolean };

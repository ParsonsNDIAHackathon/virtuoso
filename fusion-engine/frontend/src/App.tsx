...
import type { AIStatus, Alert, Assessment, Entity, FusionCandidate, FusionCluster, Region, ReplaySnapshot, Status, Viewport } from "./lib/types";
...
function AssessmentQueue({ assessments, clusters, showRejected, onToggleRejected }: { assessments: Assessment[]; clusters: FusionCluster[]; showRejected: boolean; onToggleRejected: () => void }) {
  const verdictTone = (value: Assessment["verdict"]) => value === "SUPPORTED" ? "text-command" : value === "PLAUSIBLE" ? "text-[#eab85a]" : "text-critical";
  return <Panel className="max-h-52 shrink-0 overflow-auto scrollbar"><div className="sticky top-0 z-10 flex items-center border-b border-line bg-panel px-3 py-2"><div><h2 className="font-mono text-[10px] font-semibold uppercase tracking-[.14em] text-muted">OpenAI assessments</h2><p className="text-[9px] text-muted">Graph evidence only</p></div><label className="ml-auto flex items-center gap-1 text-[9px] text-muted"><input type="checkbox" checked={showRejected} onChange={onToggleRejected} /> Show rejected</label></div>{clusters[0]?.brief && <div className="border-b border-command/30 bg-command/5 px-3 py-2"><p className="font-mono text-[9px] uppercase text-command">Top cluster · {clusters[0].modalities.join(" + ")}</p><p className="mt-1 text-[10px] text-muted">{clusters[0].brief}</p></div>}<div>{assessments.map((assessment) => <div key={assessment.id} className="border-b border-line/70 px-3 py-2"><div className="flex items-center gap-2"><span className={`font-mono text-[10px] font-bold ${verdictTone(assessment.verdict)}`}>{assessment.verdict}</span>{assessment.needs_review && <Badge className="border-[#eab85a]/50 text-[#eab85a]"><AlertTriangle size={9} /> Needs review</Badge>}<span className="ml-auto font-mono text-[9px] text-muted">{assessment.evidence_strength.toFixed(2)}</span></div><p className="mt-1 text-[10px] text-ink">{assessment.relation}</p><p className="mt-0.5 text-[10px] leading-snug text-muted">{assessment.rationale}</p></div>)}{assessments.length === 0 && <p className="px-3 py-4 text-[10px] text-muted">No AI-assessed findings yet.</p>}</div></Panel>;
}
...
function AICompareOverlay({ status, active, selections, result, pending, error, onToggle, onAnalyze, onClear }: { status?: AIStatus; active: boolean; selections: Detail[]; result: Assessment | null; pending: boolean; error?: Error | null; onToggle: () => void; onAnalyze: () => void; onClear: () => void }) {
  return <div className="absolute left-3 top-3 z-[1000] w-80 border border-line bg-panel/95 p-2 shadow-panel"><div className="flex items-center gap-2"><Button size="sm" variant={active ? "critical" : "outline"} onClick={onToggle}><Link2 size={12} /> {active ? "Selecting evidence" : "Compare with AI"}</Button><span className={`font-mono text-[9px] ${status?.configured ? "text-command" : "text-[#eab85a]"}`}>{status?.configured ? status.model : "KEY NOT SET"}</span>{selections.length > 0 && <button className="ml-auto text-[9px] text-muted hover:text-ink" onClick={onClear}>Clear</button>}</div>{active && <><p className="mt-2 text-[10px] text-muted">Select two individual GDELT, Telegram, ADS-B, or FIRMS markers.</p><div className="mt-2 grid grid-cols-2 gap-1">{[0, 1].map((index) => <div key={index} className="min-h-12 border border-line bg-black/15 p-1.5"><p className="font-mono text-[8px] uppercase text-command">{index ? "B" : "A"}</p><p className="truncate text-[10px] text-ink">{selections[index]?.title ?? "Select marker"}</p></div>)}</div><Button className="mt-2 w-full" size="sm" disabled={selections.length !== 2 || pending || !status?.configured} onClick={onAnalyze}>{pending ? "OpenAI evaluating…" : "Adjudicate evidence"}</Button></>}{error && <p className="mt-2 text-[10px] text-critical">{error.message}</p>}{result && <div className="mt-2 border-t border-line pt-2"><div className="flex items-center gap-2"><span className={`font-mono text-[10px] font-bold ${result.verdict === "SUPPORTED" ? "text-command" : result.verdict === "PLAUSIBLE" ? "text-[#eab85a]" : "text-critical"}`}>{result.verdict}</span>{result.needs_review && <Badge className="border-[#eab85a]/50 text-[#eab85a]"><AlertTriangle size={9} /> Needs review</Badge>}<span className="ml-auto font-mono text-[9px] text-muted">{result.cached ? "cached" : result.model}</span></div><p className="mt-1 text-[10px] font-semibold text-ink">{result.relation} · strength {result.evidence_strength.toFixed(2)}</p><p className="mt-1 text-[10px] leading-snug text-muted">{result.rationale}</p><p className="mt-1 text-[9px] text-critical">Limitation: {result.strongest_limitation}</p></div>}</div>;
}
...
export function App() {
  ...
  const adjudicate = useMutation({ mutationFn: () => api.adjudicate(compareSelection[0].recordRef!, compareSelection[1].recordRef!, mode, replayTime), onSuccess: (value) => { setCompareResult(value); client.invalidateQueries({ queryKey: ["fusion-candidates"] }); client.invalidateQueries({ queryKey: ["fusion-assessments"] }); client.invalidateQueries({ queryKey: ["fusion-clusters"] }); client.invalidateQueries({ queryKey: ["graph"] }); if (!live) client.invalidateQueries({ queryKey: ["replay", mode, replayTime] }); } });
  ...
  const assessed = live ? aiAssessments.data ?? [] : (replay.assessments ?? []).filter((value) =>
    showRejected || value.verdict === "SUPPORTED" || value.verdict === "PLAUSIBLE");
  ...
  const selectMap = useCallback((next: Detail) => {
    ...
    if (compareActive && next.recordRef) {
      setCompareSelection((current) => {
        if (current.some((item) => item.recordRef?.kind === next.recordRef?.kind && item.recordRef?.id === next.recordRef?.id)) return current;
  ...
  return <main className="relative z-10 min-h-screen p-2 text-ink lg:h-screen lg:overflow-hidden"><SourcesDialog open={sourcesOpen} onClose={() => setSourcesOpen(false)} /><div className="grid min-h-[calc(100vh-1rem)] grid-rows-[auto_auto_1fr] overflow-hidden border border-line bg-canvas/95 lg:h-[calc(100vh-1rem)]">
    ...
    <div className="grid min-h-0 gap-2 p-2 lg:grid-cols-[minmax(0,1fr)_390px] lg:grid-rows-[minmax(0,1fr)_240px]">
      <Panel className="relative min-h-[440px] overflow-hidden lg:min-h-0"><OperationalMap {...records} assessments={assessed} regions={regions.data ?? []} layers={layers} viewport={viewport} filterAoi={filterAoi} drawing={drawing} focus={focus} satelliteDay={live ? undefined : (replayTime ? new Date(replayTime * 1000).toISOString().slice(0, 10) : config.data?.scenario.day)} replayBounds={live ? undefined : config.data?.scenario.bbox} compareSelection={compareSelection} compareVerdict={compareResult?.verdict} onViewport={setViewport} onLayerToggle={(layer) => setLayers((value) => ({ ...value, [layer]: !value[layer] }))} onDraft={(value) => { setDraft(value); setDrawing(false); }} onSelect={selectMap} onAoiZoom={zoomToRegion} onBackgroundClick={clearMapSelection} /><AICompareOverlay status={aiStatus.data} active={compareActive} selections={compareSelection} result={compareResult} pending={adjudicate.isPending} error={adjudicate.error as Error | null} onToggle={() => { setCompareActive((value) => !value); setCompareSelection([]); setCompareResult(null); }} onAnalyze={() => adjudicate.mutate()} onClear={() => { setCompareSelection([]); setCompareResult(null); }} /><div className="pointer-events-none absolute bottom-3 left-3 border border-line bg-panel/95 px-2 py-1 font-mono text-[10px] uppercase tracking-wide text-muted">Canvas map · clusters expand on click</div></Panel>

    ...
    <div className="grid min-h-0 gap-2 p-2 lg:grid-cols-[minmax(0,1fr)_390px] lg:grid-rows-[minmax(0,1fr)_240px]">
      ...
      <aside className="relative flex min-h-0 flex-col gap-2 lg:row-span-2">
        {detail ? <><Inspector detail={detail} entity={entity.data} entityLoading={entity.isLoading} expanded onClose={() => { setDetail(null); setSourceViewer(null); }} onEmbed={(d) => setSourceViewer(d)} /><CollapsedSidebarTabs onOpen={() => { setDetail(null); setSourceViewer(null); }} /></> : <>
          ...
          <Incidents incidents={live ? (liveIncidents.data?.incidents ?? []) : (replay.incidents ?? [])} baseline={live ? liveIncidents.data?.baseline : replay.baseline} status={live ? liveIncidents.data?.status : undefined} meta={live ? { mode: "live", status: liveIncidents.data?.status ?? (liveIncidents.isLoading ? "connecting" : "waiting for first fuse"), built_at: liveIncidents.data?.built_at, assessed_through: liveIncidents.data?.assessed_through, aircraft_history: liveIncidents.data?.aircraft_history } : { mode: "replay", status: "done", assessed_through: replay.assessed_through }} onFocus={([lat, lon]) => setFocus([lat, lon, 8])} />

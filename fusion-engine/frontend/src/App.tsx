...
import type { Alert, Entity, Region, ReplaySnapshot, Status, Viewport } from "./lib/types";
...
export function App() {
  ...
  const [layers, setLayers] = useState<LayerState>({ events: true, tracks: true, firms: true, links: true, imagery: false, rf: true }); const [timelineHours, setTimelineHours] = useState(24); const [sourcesOpen, setSourcesOpen] = useState(false);
  ...
  const records = live ? { events: events.data ?? [], tracks: tracks.data ?? [], alerts: alerts.data ?? [], firms: firms.data ?? [], tails: tails.data ?? [] } : { events: replay.events, tracks: replay.tracks, alerts: replay.alerts, firms: replay.firms ?? [], tails: replay.tails ?? [] };
  ...
  const selectAlert = useCallback((alert: Alert) => {
    ...
    const next: Detail = {
      title: `Correlation score ${alert.score.toFixed(3)}`,
      lines: [alert.reason ?? "Correlation", `Δ ${alert.distance_km} km · Δt ${alert.dt_min ?? "–"} min`],
      connections: [
  ...
  return <main className="relative z-10 min-h-screen p-2 text-ink lg:h-screen lg:overflow-hidden"><SourcesDialog open={sourcesOpen} onClose={() => setSourcesOpen(false)} /><div className="grid min-h-[calc(100vh-1rem)] grid-rows-[auto_auto_1fr] overflow-hidden border border-line bg-canvas/95 lg:h-[calc(100vh-1rem)]">
    ...
    <div className="grid min-h-0 gap-2 p-2 lg:grid-cols-[minmax(0,1fr)_390px] lg:grid-rows-[minmax(0,1fr)_240px]">
      <Panel className="relative min-h-[440px] overflow-hidden lg:min-h-0"><OperationalMap {...records} regions={regions.data ?? []} layers={layers} viewport={viewport} filterAoi={filterAoi} drawing={drawing} focus={focus} satelliteDay={live ? undefined : (replayTime ? new Date(replayTime * 1000).toISOString().slice(0, 10) : config.data?.scenario.day)} replayBounds={live ? undefined : config.data?.scenario.bbox} onViewport={setViewport} onLayerToggle={(layer) => setLayers((value) => ({ ...value, [layer]: !value[layer] }))} onDraft={(value) => { setDraft(value); setDrawing(false); }} onSelect={selectMap} onAoiZoom={zoomToRegion} onBackgroundClick={clearMapSelection} /><div className="pointer-events-none absolute bottom-3 left-3 border border-line bg-panel/95 px-2 py-1 font-mono text-[10px] uppercase tracking-wide text-muted">Canvas map · clusters expand on click</div></Panel>

...
import { Crosshair, ExternalLink, Info, Link2, MapPin, Newspaper, Pause, Plane, Play, Radio, RefreshCw, ScanSearch, Trash2, Users, X } from "lucide-react";
...
function MapKey({ label, color, detail, action = "Click center to zoom or analyze" }: { label: string; color: string; detail: string; action?: string }) {
  return <div className="flex items-center gap-2 border-l border-line px-3 py-0.5 text-[10px]" title={detail}>
    <span aria-label={`${label} map marker`} className="h-2.5 w-2.5 shrink-0 rounded-full border border-black/30" style={{ background: color }} /><span className="font-semibold text-ink">{label}</span><span className="text-muted">{action}</span>
  </div>;
}
...
function Inspector({ detail, entity, entityLoading = false, onClose, onEmbed }: { detail: Detail; entity?: Entity; entityLoading?: boolean; onClose?: () => void; onEmbed?: (d: Detail) => void }) {
  return <Panel className="flex h-60 min-h-0 shrink-0 flex-col overflow-hidden p-3"><div className="mb-2 flex items-center gap-2"><h2 className="font-mono text-[10px] font-semibold uppercase tracking-[.14em] text-muted">Selected source</h2>{onClose && <button aria-label="Close selected source" onClick={onClose} className="ml-auto grid h-6 w-6 place-items-center border border-line text-muted hover:text-ink"><X size={13} /></button>}</div><div className="min-h-0 flex-1 overflow-auto scrollbar">{detail ? <div className="space-y-1 text-xs leading-relaxed"><p className="font-semibold text-ink">{detail.title}</p>{detail.lines.map((line, index) => <p key={index} className="text-muted">{line}</p>)}{detail.connections && <CorrelationEvidence connections={detail.connections} />}{detail.entityId && <GraphEvidence entity={entity} loading={entityLoading} />}{detail.href && <SourcePreview href={detail.href} label={detail.hrefLabel ?? "Source"} onEmbed={onEmbed && canPreviewSource(detail.href) ? () => onEmbed(detail) : undefined} />}</div> : <p className="text-xs text-muted">Select an alert or map element to inspect its operational context.</p>}</div></Panel>;
}
...
export function App() {
  ...
  return <main className="relative z-10 min-h-screen p-2 text-ink lg:h-screen lg:overflow-hidden"><SourcesDialog open={sourcesOpen} onClose={() => setSourcesOpen(false)} /><div className="grid min-h-[calc(100vh-1rem)] grid-rows-[auto_auto_1fr] overflow-hidden border border-line bg-canvas/95 lg:h-[calc(100vh-1rem)]">
    ...
    <ResizableWorkspace
      ...
      timeline={<Panel className="relative min-h-[240px] min-w-0 overflow-hidden lg:col-start-1 lg:row-start-2 lg:min-h-0"><ActivityTimeline timeline={live ? liveTimeline.data : replayTimeline.data} activeTime={live ? undefined : replayTime} mode={live ? "live" : "replay"} onSeek={live ? undefined : (time) => { setPlaying(false); setReplayTime(time); }} /></Panel>}
    />
  </div></main>;
}

...
import type { AIStatus, Alert, Assessment, Entity, EntityNode, FusionCandidate, FusionCluster, RecordRef, Region, ReplaySnapshot, Vessel, Status, Viewport } from "./lib/types";
...
function SourceViewer({ detail, onClose }: { detail: Detail; onClose: () => void }) {
  ...
  return <Panel panelId="source-viewer" className="absolute inset-0 z-20 flex min-h-0 flex-col overflow-hidden border-command/60 bg-canvas shadow-2xl">
...
function AssessmentQueue({ assessments, clusters, showRejected, onToggleRejected }: { assessments: Assessment[]; clusters: FusionCluster[]; showRejected: boolean; onToggleRejected: () => void }) {
  return <Panel panelId="assessments" className="max-h-[60vh] shrink-0 overflow-auto scrollbar">
...
export function App() {
  ...
  const [layers, setLayers] = useState<LayerState>({ events: true, tracks: true, ais: true, firms: true, sar: false, links: true, imagery: false, rf: true, incidents: true }); const [timelineHours, setTimelineHours] = useState(24); const [sourcesOpen, setSourcesOpen] = useState(false);
  ...
  // Right column tabs, organised by the analyst's question: Incidents (what changed and why),
  // Evidence (what links the records), Watch (what am I watching). Actions switch the tab for you.
  type SideTab = "incidents" | "evidence" | "watch";
  const [sideTab, setSideTabState] = useState<SideTab>(() => { try { const v = localStorage.getItem("layout.sideTab"); return v === "evidence" || v === "watch" ? v : "incidents"; } catch { return "incidents"; } });
  const setSideTab = (t: SideTab) => { setSideTabState(t); try { localStorage.setItem("layout.sideTab", t); } catch { /* ignore */ } };
  ...
  const records = live ? { vessels: ais.data?.vessels ?? [], events: events.data ?? [], tracks: tracks.data ?? [], alerts: alerts.data ?? [], firms: firms.data ?? [], sar: [], tails: tails.data ?? [], sarCore: [] } : { vessels: [] as Vessel[], events: replay.events, tracks: replay.tracks, alerts: replay.alerts, firms: replay.firms ?? [], sar: replay.sar ?? [], tails: replay.tails ?? [], sarCore: replay.sar_core ?? [] };
  ...
  return <main className="relative z-10 min-h-screen p-2 text-ink lg:h-screen lg:overflow-hidden"><SourcesDialog open={sourcesOpen} onClose={() => setSourcesOpen(false)} /><div className="grid min-h-[calc(100vh-1rem)] grid-rows-[auto_auto_1fr] overflow-hidden border border-line bg-canvas/95 lg:h-[calc(100vh-1rem)]">
    ...
    <div className="grid min-h-0 gap-2 p-2" style={wide ? { gridTemplateColumns: `minmax(0,1fr) ${sidebarW}px`, gridTemplateRows: `minmax(0,1fr) ${timelineH}px` } : undefined}>
      <Panel className="relative min-h-[440px] overflow-hidden lg:min-h-0"><OperationalMap {...records} live={live} assessments={assessed} incidents={live ? (liveIncidents.data?.incidents ?? []) : (replay.incidents ?? [])} selectedIncident={incidentSel} onIncidentSelect={(id) => { setIncidentSel(id); setDetail(null); if (id) setSideTab("incidents"); }} regions={regions.data ?? []} layers={layers} viewport={viewport} filterAoi={filterAoi} drawing={drawing} focus={focus} satelliteDay={live ? undefined : (replayTime ? new Date(replayTime * 1000).toISOString().slice(0, 10) : config.data?.scenario.day)} replayBounds={live ? undefined : config.data?.scenario.bbox} compareSelection={compareSelection} compareVerdict={compareResult?.verdict} compareArticleMatch={compareResult?.has_article_match} onViewport={setViewport} onLayerToggle={(layer) => setLayers((value) => ({ ...value, [layer]: !value[layer] }))} onDraft={(value) => { setDraft(value); setDrawing(false); setSideTab("watch"); }} onSelect={selectMap} onAoiZoom={zoomToRegion} onBackgroundClick={clearMapSelection} /><AICompareOverlay status={aiStatus.data} active={compareActive} selections={compareSelection} result={compareResult} pending={adjudicate.isPending} error={adjudicate.error as Error | null} onToggle={() => { setCompareActive((value) => !value); setCompareSelection([]); setCompareResult(null); }} onAnalyze={() => { const [left, right] = compareSelection; if (!left?.recordRef || !right?.recordRef) return; setPlaying(false); adjudicate.mutate({ left: left.recordRef, right: right.recordRef, mode, time: live ? null : left.replayTime ?? replay.t, force: Boolean(compareResult) }); }} onClear={() => { setCompareSelection([]); setCompareResult(null); }} /><div className="pointer-events-none absolute bottom-3 left-3 border border-line bg-panel/95 px-2 py-1 font-mono text-[10px] uppercase tracking-wide text-muted">Canvas map · clusters expand on click</div></Panel>
      <aside className="sidebar-resizable relative flex min-h-0 flex-col gap-2 lg:row-span-2"><PanelGroup>
        ...
        {sourceList && <SourceRecords source={sourceList} engineCount={sourceList === "social" ? (currentStatus.sources?.social ?? currentStatus.sources?.telegram)?.count : sourceList === "ais" ? ais.data?.source?.count : currentStatus.sources?.[sourceList]?.count} events={records.events} tracks={records.tracks} vessels={records.vessels} firms={records.firms} resetKey={sourceListKey} onClose={() => setSourceList(null)} onResetMap={() => { setFocus([35, 10, 2]); setDetail(null); }}
          onOpen={(d) => { setSourceViewer(null); setDetail(d); if (d.point) setFocus([d.point[0], d.point[1], 8]); }} />}
        {!detail && !sourceList && <p className="px-1 font-mono text-[9px] uppercase tracking-[.14em] text-muted">Inspector · click a map marker, a source chip, an evidence record, or a graph item</p>}
        {(() => { const incs = live ? (liveIncidents.data?.incidents ?? []) : (replay.incidents ?? []); const fresh = incs.filter((x) => x.state === "new_change").length; const findings = assessed.filter((a) => a.verdict === "SUPPORTED" || a.verdict === "PLAUSIBLE").length; const src = Object.values(currentStatus.sources ?? {}); const bad = src.filter((x) => x.state === "error").length;
          const tabs: Array<{ id: SideTab; label: string; count: string; title: string }> = [
            { id: "incidents", label: "Incidents", count: incs.length ? `${incs.length}${fresh ? ` · ${fresh} new` : ""}` : "0", title: "What changed, and why: incidents formed from the streams, with the full report" },
            { id: "evidence", label: "Evidence", count: findings ? `${findings} finding${findings === 1 ? "" : "s"}` : `${candidates.length} cand.`, title: "What links the records: AI assessments, fusion candidates, curated evidence" },
            { id: "watch", label: "Watch", count: `${(regions.data ?? []).length} area${(regions.data ?? []).length === 1 ? "" : "s"}${bad ? ` · ${bad} source error${bad === 1 ? "" : "s"}` : ""}`, title: "What you are watching: areas of interest and source health" }];
          return <div role="tablist" aria-label="Right column sections" className="flex shrink-0 border border-line bg-panel/95">{tabs.map((t) => <button key={t.id} role="tab" aria-selected={sideTab === t.id} title={t.title} onClick={() => setSideTab(t.id)}
            className={`flex-1 border-b-2 px-2 py-1.5 text-left ${sideTab === t.id ? "border-command bg-white/[.05] text-ink" : "border-transparent text-muted hover:text-ink"}`}>
            <span className="block font-mono text-[10px] font-semibold uppercase tracking-[.14em]">{t.label}</span><span className={`block font-mono text-[9px] ${t.id === "watch" && bad ? "text-critical" : sideTab === t.id ? "text-command" : "text-muted"}`}>{t.count}</span></button>)}</div>; })()}
        {sideTab === "incidents" && <>
        ...
        {sideTab === "watch" && <>
          ...
          <Panel panelId="source-health" className="p-3"><h2 className="font-mono text-[10px] font-semibold uppercase tracking-[.14em] text-muted">Source health</h2>
            <ul className="mt-1 space-y-0.5 font-mono text-[10px]">{Object.entries(currentStatus.sources ?? {}).filter(([k]) => !k.startsWith("social:") && k !== "telegram").map(([k, v]) => <li key={k} className="flex gap-2"><span className={`w-16 shrink-0 uppercase ${v.state === "error" ? "text-critical" : v.state === "ready" ? "text-command" : "text-[#eab85a]"}`}>{v.state}</span><span className="w-16 shrink-0 text-ink">{k}</span><span className="min-w-0 truncate text-muted" title={v.detail ?? ""}>{v.count != null ? `${v.count.toLocaleString()} · ` : ""}{v.detail ?? ""}</span></li>)}</ul>
            <p className="mt-1 text-[9px] text-muted">Click a chip in the strip above the map to list a source's records.</p></Panel>
        </>}
